...
from processor.processor_stage1 import do_train_stage1
from processor.processor_octar import do_train_stage2
import random
import torch
import numpy as np
import os
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description="OCTAR training")
    parser.add_argument(
        "--config_file", default="configs/person/octar_occluded_duke.yml", help="path to config file", type=str
    )
    ...
    if cfg.SOLVER.STAGE1.SKIP:
        if not cfg.SOLVER.STAGE1.WEIGHT:
            raise ValueError("SOLVER.STAGE1.SKIP is True, but SOLVER.STAGE1.WEIGHT is empty.")
        logger.info("Skipping Stage1 and loading Stage1 checkpoint: {}".format(cfg.SOLVER.STAGE1.WEIGHT))
